...
class PackageModel {
    ...
    static async findAllWithDetails(filters?: { categoryId?: string; creatorId?: string; search?: string; status?: string }) {
        try {
            ...
            throw { message: "Failed to retrieve packages with details", status: 500 };
        }
    }
    ...
    static async findById(packageId: string): Promise<PackageResponse> {
        try {
            ...
            const pkg = await this.collection().findOne({ _id: new ObjectId(packageId) });
    ...
    static async deleteById(packageId: string) {
        try {
            ...
            const result = await this.collection().deleteOne({ _id: new ObjectId(packageId) });
     ...
     */
    static async findByCategoryId(categoryId: string): Promise<PackageResponse[]> {
        try {
     ...
     */
    static async count(filters?: { categoryId?: string; creatorId?: string }): Promise<number> {
        try {
            const query: any = {};
     ...
     */
    static async findAllPublished(filters?: { categoryId?: string; search?: string }) {
        try {
    ...
    static async togglePublishStatus(packageId: string, isPublished: boolean) {
        try {
            ...
            return {
                success: true,
                message: `Package ${isPublished ? 'published' : 'unpublished'} successfully`
            };
        } catch (error) {
            if (error && typeof error === 'object' && 'status' in error) {
                throw error;
     ...
     */
    static async findByCreatorIdAll(creatorId: string, publishedOnly: boolean = false): Promise<PackageResponse[]> {
        try {
            this.validateObjectId(creatorId);
     ...
     */
    static async findPublishedByCategoryId(categoryId: string): Promise<PackageResponse[]> {
        try {
            this.validateObjectId(categoryId);
            ...
            throw { message: "Failed to find published packages by category", status: 500 };
        }
    }
