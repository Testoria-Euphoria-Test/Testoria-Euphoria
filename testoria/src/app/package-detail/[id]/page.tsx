...
export default function PackageDetailPage() {
  ...
  // Separate state for description editor to prevent lag
  ...
  const [currentQuestionForImage, setCurrentQuestionForImage] = useState<
    string | null
  >(null);
  ...
  const loadPackageDetail = useCallback(async () => {
    ...
    try {
      ...
      const packageResponse = await fetch(
        `/api/packages/${packageId}?withDetails=true`,
        {
          method: "GET",
          credentials: "include",
        }
      ...
      // Initialize editor description
      setEditorDescription(pkg.description || "");

      // Check if user is owner
      setIsOwner(
        pkg.creatorId === currentUserId || authData.data.userRole === "admin"
      );
  ...
  const handleEditPackage = () => {
    ...
    // Initialize editor description with current package description
    setEditorDescription(packageData?.description || "");
  };
  ...
  const handleSavePackage = async () => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Gagal memperbarui paket";
      toast.error(errorMessage);
    } finally {
  ...
  const handleCancelEditPackage = () => {
    ...
    // Reset editor description to original value
    setEditorDescription(packageData?.description || "");
  };
  ...
  const handlePackageImageFileSelect = (
  ...
  const handleRemovePackageImage = async (index: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn(
          "Failed to delete from Cloudinary, but continuing with local removal"
        );
      }
  ...
  const handleSaveQuestion = async () => {
    ...
    try {
      ...
      setQuestions((prev) =>
        prev.map((q) =>
          q._id === editingQuestionId ? { ...q, ...editQuestionData } : q
        )
      );
  ...
  const handleProcessAI = async () => {
    ...
    try {
      ...
      toast.success(
        `AI processing completed! Generated ${questionsResult.questionsCreated} questions`
      );
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Failed to process AI";
      toast.error(errorMessage);
    } finally {
      setIsProcessing(false);
    }
  ...
  const handleQuestionImageFileSelect = (
    questionId: string,
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    const file = event.target.files?.[0];
                    ...
                    {isEditingPackage ? (
                      <WysiwygEditor
                        ...
                        onChange={(value) => {
                          ...
                          // Update package data with a slight delay to prevent too frequent updates
                          setTimeout(() => {
                            setEditPackageData((prev) => ({
                              ...prev,
                              description: value,
                      ...
                      <div
                        className="text-gray-600 wysiwyg-content-view"
                    ...
                    {isEditingPackage ? (
                      <div className="space-y-4">
                        <p className="text-sm text-gray-500">
                          Upload images that will be displayed as a slideshow on
                          your package card.
                        </p>
                        ...
                        editPackageData.images.length > 0 ? (
                              ...
                              >
                                ...
                                <div className="flex flex-col gap-1">
                                  <button
                                    type="button"
                                    onClick={() =>
                                      handleMovePackageImageUp(index)
                                    }
                                    disabled={index === 0}
                                  ...
                                  <button
                                    type="button"
                                    onClick={() =>
                                      handleMovePackageImageDown(index)
                                    }
                                    disabled={
                                      index ===
                                      (editPackageData.images?.length || 0) - 1
                                    }
                                    className="p-1 text-gray-400 hover:text-gray-600 disabled:opacity-50 disabled:cursor-not-allowed"
                                    title="Move down"
                                  >
                                ...
                                <button
                                  type="button"
                                  onClick={() =>
                                    handleRemovePackageImage(index)
                                  }
                          ...
                          <div className="text-center py-4 border-2 border-dashed border-gray-300 rounded-lg">
                            ...
                            <p className="text-xs text-gray-400">
                              Package will use default cover
                            </p>
                          </div>
                        )}
                      </div>
                        ...
                        {packageData.images && packageData.images.length > 0 ? (
                          <div className="space-y-2">
                            <p className="text-sm text-gray-600 mb-2">
                              {packageData.images.length} image(s) configured
                              for slideshow
                            </p>
                          ...
                          <p className="text-gray-500 text-sm">
                            No images configured - using default cover
                          </p>
                        )}
                      </div>
                    )}
                    ...
                    <div>
                      <label className="flex items-center space-x-2">
                        <input
                          ...
                          checked={
                            isEditingPackage
                              ? editPackageData.isPublished || false
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                      ...
                      <p className="text-xl font-bold text-purple-900">
                        {packageData.sourcePdf?.length || 0}
                      </p>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <p className="text-gray-900">
                      {packageData.categoryName || "Tanpa Kategori"}
                    </p>
                  </div>
                              ...
                              {question.passage && (
                                <div className="mb-4 p-3 bg-purple-50 rounded-lg border border-purple-200">
                                  <p className="text-sm text-purple-800">
                                    <span className="font-medium">Bacaan:</span>{" "}
                                    {question.passage}
                                  </p>
                                </div>
                              )}
                                ...
                                question.images.length > 0 ? (
                                  ...
                                  <div className="text-center py-4 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                                    ...
                                    {isOwner && question.imagePrompt && (
                                      <p className="text-xs text-gray-400 mt-1">
                                        Click "Generate" to create an image from
                                        the prompt
                                      </p>
                                    )}
                                  </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-col space-y-3">
                  <input
                    ...
                    accept="image/*"
                    onChange={(e) =>
                      handleQuestionImageFileSelect(currentQuestionForImage, e)
                    }
                    className="hidden"
              ...
              {(() => {
                const question = questions.find(
                  (q) => q._id === currentQuestionForImage
                );
                return (
                  question?.imagePrompt && (
                    <div className="border-t pt-4">
                      <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                        ...
                        <button
                          onClick={() =>
                            handleGenerateImage(
                              currentQuestionForImage,
                              question.imagePrompt!
                            )
                          }
                          disabled={
                            isGeneratingImage || isUploadingQuestionImage
                          }
                        ...
                        <p className="text-xs text-purple-600 mt-2 text-center">
                          AI will generate an image based on the prompt and
                          automatically add it to the question
                        </p>
                      </div>
                    </div>
                  )
