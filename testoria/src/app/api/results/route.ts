...
export async function POST(req: NextRequest) {
    try {
        ...
        const answers = await UserAnswerModel.findByUserAndPackage(userId, packageId);
        ...
        const totalWrong = answers.filter((a: any) => !a.isCorrect).length;
        ...
        try {
            ...
            if (score >= 80) {
                ...
                feedback = "Good effort! Consider reviewing the topics you missed to strengthen your knowledge.";
            } else {
                feedback = "Keep practicing! Review the material and try again to improve your understanding.";
            }
        }
        ...
        const existingResult = await ResultModel.findByUserAndPackage(userId, packageId);
        if (existingResult) {
            return NextResponse.json({ 
